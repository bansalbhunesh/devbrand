...
registerWorkflow("transform_pr_webhook", AnalyzePRWorkflow);
registerWorkflow("transform_pr", AnalyzePRWorkflow); // Support both job types
...
export async function drainQueueTick(limit = 3): Promise<{
  ...
  for (const job of claimed) {
    try {
      if (job.type === "transform_pr_webhook" || job.type === "transform_pr") {
        const payload = job.payload ?? {};
        let prUrl = payload.prUrl;

        // Webhook payload handling
        if (!prUrl && payload.owner && payload.repo && payload.prNumber) {
          prUrl = `https://github.com/${payload.owner}/${payload.repo}/pull/${payload.prNumber}`;
        }
