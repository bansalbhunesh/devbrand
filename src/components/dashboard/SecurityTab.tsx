...
import { Shield, AlertTriangle, ShieldCheck, Clock, ExternalLink } from "lucide-react";
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center h-[50vh] text-center space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">Security logs are restricted to Pro accounts.</p>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Real-time audit logs of platform security events.</p>
        </div>
      ...
      <div className="grid gap-4">
        {events?.length === 0 ? (
          <div className="p-12 border-2 border-dashed border-border rounded-3xl flex flex-col items-center text-center">
            ...
            <p className="font-bold text-muted-foreground">No security events recorded</p>
          </div>
        ) : (
            ...
            >
              <div className="flex items-start gap-4">
                <div className={cn(
                  ...
                  event.type === "login_attempt" ? "bg-blue-500/10 text-blue-500" :
                  event.type === "rate_limit_exceeded" ? "bg-yellow-500/10 text-yellow-500" :
                  "bg-red-500/10 text-red-500"
                )}>
                  {event.type === "login_attempt" ? <ShieldCheck className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
                </div>
                  ...
                  {event.details && (
                    <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {Object.entries(event.details).map(([k, v]) => (
                        <div key={k} className="px-2 py-1 rounded bg-muted/50 border border-border/50 text-[9px] font-mono">
                          <span className="opacity-50 lowercase">{k}:</span> {String(v)}
                        </div>
                      ))}
                    </div>
                  )}
