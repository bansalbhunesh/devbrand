...
export function HistoryCard({ output, _userId, onQueryInvalidate }: { output: any; _userId: string; onQueryInvalidate: () => void }) {
  ...
  const handleTogglePublic = async () => {
    setToggling(true);
    try {
      await toggleOutputVisibility({ data: { outputId: output.id, isPublic: !output.isPublic } });
      onQueryInvalidate();
    ...
    <div className="group rounded-2xl border border-border bg-muted/20 hover:border-border-strong transition p-6 relative overflow-hidden flex flex-col">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <span className="text-[10px] uppercase tracking-[0.2em] text-blue-500 font-black">{output.category}</span>
        ...
        <div className="flex items-center gap-1">
          ...
          >
            {output.isPublic ? <Eye className="h-3.5 w-3.5 text-blue-500" /> : <EyeOff className="h-3.5 w-3.5" />}
          </button>
          <button 
            ...
            <DialogContent className="max-w-4xl max-h-[85vh] overflow-hidden flex flex-col p-0 gap-0 border-border bg-background">
              <DialogHeader className="p-6 border-b border-border">
                <div className="flex items-center gap-2 text-xs text-muted-foreground mb-1 uppercase tracking-widest font-bold">
                  <Sparkles className="h-3.5 w-3.5 text-blue-500" /> Verifiable Evidence Layer
                </div>
                <DialogTitle className="text-2xl font-bold">{output.prTitle || "PR Analysis"}</DialogTitle>
                <DialogDescription className="text-xs">
                  This impact story is backed by {citations.length} specific code citations from the git log.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="flex-1 overflow-y-auto grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border">
                <div className="p-8 space-y-6">
                  <div className="space-y-4">
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Generated Narrative</h4>
                  ...
                  <div className="p-4 rounded-xl border border-blue-500/10 bg-blue-500/5">
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-500 mb-2">Resume Impact</h4>
                    <p className="text-sm font-mono text-blue-500/80 leading-relaxed">{output.resumeBullet}</p>
                  </div>
                </div>
                
                <div className="p-8 bg-muted/10 space-y-6">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Source Evidence</h4>
                  <div className="space-y-4">
                    {citations.map((cite: any, i: number) => (
                      <div key={i} className="group/cite space-y-2">
                        <div className="flex items-center gap-2 text-xs font-bold">
                          <span className="text-blue-500">[{i + 1}]</span>
                          <span className="font-mono text-muted-foreground">{cite.file || cite.path}</span>
                          <span className="ml-auto opacity-30 text-[10px] font-mono">{cite.sha?.slice(0, 7) || "a4f1c2"}</span>
                        </div>
                        <div className="p-3 rounded-lg border border-border bg-background font-mono text-[11px] leading-relaxed text-muted-foreground">
                          {cite.context || cite.description || "Architectural shift in handling concurrent transactions."}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="p-4 border-t border-border bg-muted/20 flex justify-between items-center">
                ...
                <button 
                  onClick={() => handleCopyText(output.linkedinPost1, "modal-copy")}
                ...
                >
                  {copied === "modal-copy" ? <Check className="h-3 w-3" /> : <ClipboardCopy className="h-3 w-3" />}
                  Copy Narrative
                </button>
              </div>
            </DialogContent>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-border/50 mt-auto">
        <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">
          {new Date(output.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
        </span>
        <button 
        ...
        >
          {copied === "post" ? (
            ...
            <><ClipboardCopy className="h-3.5 w-3.5 group-hover/btn:scale-110 transition" /> Copy post</>
          )}
        </button>
      </div>
    </div>
