...
export function TeamsTab({ user, handleUpgrade }: TeamsTabProps) {
  const demoTeam = [
    { name: "Alex Chen", impact: 92, velocity: "+12%", avatar: "AC", status: "Lead" },
    { name: "Sarah Miller", impact: 88, velocity: "+5%", avatar: "SM", status: "Senior" },
    { name: "Jamie V.", impact: 84, velocity: "+8%", avatar: "JV", status: "Member" },
    { name: "Chris P.", impact: 79, velocity: "-2%", avatar: "CP", status: "Member" },
  ];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Compare impact velocity and sync architectural signals across your org.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex flex-col items-end px-4 py-1 border-r border-border">
            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Team Velocity</span>
      ...
      {user.plan !== "pro" ? (
        <div className="relative group">
          <div className="absolute inset-0 bg-background/60 backdrop-blur-[2px] z-20 rounded-[2rem] border-2 border-dashed border-border flex flex-col items-center justify-center p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-sm mb-8">
              Benchmark your engineering impact against your team, automate manager reporting, and sync high-signal architectural shifts.
            </p>
            <button 
          ...
          <div className="opacity-40 select-none pointer-events-none grid md:grid-cols-[1.5fr_1fr] gap-8">
                  ...
                  <div key={i} className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-muted-foreground uppercase font-black">{member.status}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-mono font-bold text-blue-500">{member.impact}</div>
                      <div className={cn("text-[10px] font-bold", member.velocity.startsWith("+") ? "text-green-500" : "text-red-500")}>
                        {member.velocity}
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-8">
              <div className="rounded-3xl border border-border bg-muted/20 p-8">
                <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-muted-foreground mb-6">Aggregate Velocity</h4>
              ...
              <div className="rounded-3xl border border-border bg-blue-500/5 p-8 border-blue-500/10">
                 <div className="flex items-center gap-2 text-blue-500 mb-2">
                    ...
                    <span className="text-xs font-black uppercase tracking-widest">Top Signal</span>
                 </div>
                 <p className="text-sm font-medium leading-relaxed">
                   Your team's architectural focus shifted 24% towards <span className="text-blue-500 font-bold">System Reliability</span> this month.
                 </p>
              </div>
            </div>
          </div>
        ...
        <div className="py-32 text-center border-2 border-dashed border-border rounded-[2.5rem] bg-muted/10">
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-[320px] mx-auto leading-relaxed">
            Create a team to benchmark impact, sync architectural signals, and automate performance reviews.
          </p>
