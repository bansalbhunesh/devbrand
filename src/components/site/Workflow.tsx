...
import { GitBranch, ListChecks, Share2, ArrowRight, Github } from "lucide-react";
...
const steps = [
  {
    ...
    title: "Ingest",
    description: "Connect your GitHub account and select the repositories that hold your best architectural work.",
    color: "blue",
    mockup: (
      <div className="space-y-3 p-6 bg-muted/20 rounded-2xl border border-white/5 font-mono text-[11px]">
        <div className="flex items-center justify-between p-3 rounded-xl bg-background border border-border">
          ...
          <div className="px-2 py-0.5 rounded bg-blue-500/10 text-blue-500 border border-blue-500/20 text-[9px]">CONNECTED</div>
        </div>
        <div className="flex items-center justify-between p-3 rounded-xl bg-background/50 border border-border/50 opacity-60">
          ...
          <button className="px-2 py-0.5 rounded border border-border hover:bg-white/5 transition">CONNECT</button>
        </div>
        <div className="flex items-center justify-between p-3 rounded-xl bg-background/50 border border-border/50 opacity-60">
          <div className="flex items-center gap-3">
            <Github className="h-4 w-4 opacity-50" />
  ...
  },
  {
    ...
    title: "Curate",
    description: "Select the specific Pull Requests that showcase your growth. DevBrand analyzes the raw diffs automatically.",
    color: "purple",
    mockup: (
      <div className="space-y-3 p-6 bg-muted/20 rounded-2xl border border-white/5 font-mono text-[11px]">
        <div className="flex items-center gap-3 p-3 rounded-xl bg-background border border-purple-500/30">
          ...
          <div className="flex-1">
            <div className="text-foreground">feat: implement retry-worker jitter</div>
            <div className="text-[9px] text-muted-foreground">#1428 · payments-worker</div>
          </div>
        </div>
        <div className="flex items-center gap-3 p-3 rounded-xl bg-background border border-purple-500/30">
          <div className="h-4 w-4 rounded border border-purple-500 bg-purple-500/20 flex items-center justify-center">
          ...
          <div className="flex-1">
            <div className="text-foreground">refactor: move auth to edge worker</div>
            <div className="text-[9px] text-muted-foreground">#812 · edge-gateway</div>
          </div>
        </div>
        <div className="flex items-center gap-3 p-3 rounded-xl bg-background/50 border border-border/50 opacity-60">
          ...
          <div className="flex-1">
            ...
            <div className="text-[9px] text-muted-foreground">#1429 · payments-worker</div>
          </div>
        </div>
      </div>
    )
  },
  {
    ...
    title: "Export",
    description: "Convert raw code into strategic narratives for LinkedIn, Resume bullets, or your personal Signal Profile.",
    color: "green",
    mockup: (
      <div className="p-6 bg-muted/20 rounded-2xl border border-white/5 font-mono text-[11px] space-y-4">
        <div className="p-4 rounded-xl bg-background border border-green-500/20">
          <div className="text-[9px] text-green-500/60 mb-2 uppercase tracking-widest font-bold italic">Generated Signal</div>
          <p className="text-foreground/80 leading-relaxed italic">"Migrated session handling to a stateless JWT pipeline, reducing latency p95 by 38%..."</p>
        </div>
...
export function Workflow() {
  return (
    <section id="workflow" className="relative border-t border-border overflow-hidden snap-start snap-always w-full">
          ...
          {steps.map((step, i) => (
            ...
            >
              <div className="space-y-4">
                <div className={`h-12 w-12 rounded-2xl bg-${step.color}-500/10 border border-${step.color}-500/20 flex items-center justify-center text-${step.color}-500 shadow-[0_0_20px_rgba(var(--${step.color}-500-rgb),0.1)]`}>
                  {step.icon}
                </div>
