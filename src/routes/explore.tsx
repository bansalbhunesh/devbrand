...
function ExplorePage() {
  const [tab, setTab] = React.useState<"global" | "impact" | "structural">("global");
  ...
  const filteredFeed = React.useMemo(() => {
    ...
    if (tab === "impact") return feed.feed.filter(i => (i.impactScore ?? 0) >= 80);
    if (tab === "structural") return feed.feed.filter(i => i.category === "Structural" || i.category === "Refactor" || i.category === "Architecture");
    return feed.feed;
  }, [feed, tab]);
      ...
      <div className="relative z-10 mx-auto max-w-7xl py-16 px-6 sm:py-24">
        <header className="mb-16">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl font-medium">
            Discover verifiable architectural transformations and high-impact engineering stories verified by DevBrand AI.
          </p>
        </header>
        ...
        <div className="flex items-center gap-8 mb-16 border-b border-border relative">
          {(["global", "impact", "structural"] as const).map((t) => (
            ...
            >
              {t === "global" ? "Global Stream" : t === "impact" ? "Elite Impact" : "Structural Logic"}
              {tab === t && (
                <motion.div 
            ...
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold flex items-center gap-3">
                ...
                <span className="uppercase tracking-widest text-xs font-black">
                  {tab === "global" ? "Latest Transformations" : tab === "impact" ? "Elite Performance" : "Invisible Infrastructure"}
                </span>
              </h2>
            ...
            {!filteredFeed.length && !isLoading ? (
              <div className="py-32 rounded-[2.5rem] border-2 border-dashed border-border bg-muted/10 text-center">
                ...
                <p className="text-sm text-muted-foreground max-w-xs mx-auto leading-relaxed">
                  No public engineering transformations found in this sector yet.
                </p>
              </div>
            ) : (
                        ...
                        <div className="flex items-center justify-between mb-6">
                          <div className="flex items-center gap-3">
                            <div className="h-8 w-8 rounded-lg bg-background border border-border grid place-items-center font-bold text-[10px]">
                              {item.user?.githubLogin?.slice(0, 2).toUpperCase()}
                            </div>
                            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{item.user?.githubLogin}</span>
                          </div>
                          <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-blue-500/5 text-blue-500 border border-blue-500/10 text-[10px] font-black">
                            <TrendingUp className="h-3 w-3" /> {item.impactScore}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-6 border-t border-border mt-auto">
                          <div className="flex gap-2">
                            {item.stack?.slice(0, 2).map(s => (
                              <span key={s} className="text-[9px] font-black px-2 py-1 rounded bg-muted-foreground/10 uppercase tracking-wider">
                                {s}
                              </span>
                            ))}
                          </div>
          ...
          <div className="space-y-16">
            ...
            <div className="space-y-8">
              <h2 className="text-xl font-bold flex items-center gap-3">
                ...
                <span className="uppercase tracking-widest text-xs font-black">Top Velocity</span>
              </h2>
                ...
                {feed?.topEngineers?.map((eng, i) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        <img src={eng.avatarUrl ?? ""} className="h-8 w-8 rounded-lg shadow-xl shadow-black/20" alt="" />
                        <span className="text-sm font-bold truncate max-w-[100px]">{eng.githubLogin}</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-[12px] font-black text-blue-500 tracking-tighter">{Math.round(eng.avgImpact)}%</div>
                      <div className="text-[8px] text-muted-foreground uppercase font-black tracking-widest opacity-60">Avg Impact</div>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            <div className="space-y-8">
              <h2 className="text-xl font-bold flex items-center gap-3">
                ...
                <span className="uppercase tracking-widest text-xs font-black">Critical Roasts</span>
              </h2>
                ...
                {feed?.topRoasts?.length ? feed.topRoasts.map((roast) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest opacity-60">@{roast.githubUsername}</span>
                      <div className={cn(
                    ...
                    <div className="flex items-center justify-between">
                       <div className="text-[10px] font-mono font-bold text-red-500/60 uppercase tracking-tighter">SCORE: {roast.roastData.roast_score}/100</div>
                  ...
                  <div className="p-8 rounded-2xl border border-dashed border-border text-center">
                    <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 leading-relaxed">
                      No public roasts found. <br /> Roast a repo to ignite the feed.
                    </p>
                  </div>
                )}
              </div>
            ...
            {/* Promo CTA */}
            <Link to="/" className="block p-8 rounded-[2rem] bg-foreground text-background group overflow-hidden relative">
