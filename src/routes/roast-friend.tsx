          ...
          <div className="mx-auto max-w-3xl px-6 relative">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-16">
              ...
              <p className="text-lg text-muted-foreground max-w-lg mx-auto">
                Enter their GitHub username. We'll generate an evidence-backed roast and let you tag them on social media. No mercy.
              </p>
            </motion.div>

            {/* Input */}
            {!roastData && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="max-w-md mx-auto space-y-6">
              ...
              {roastData && (
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    roastData.criticality === "NUCLEAR" ? "bg-red-500/10 border-red-500/40" : "bg-muted/20 border-border"
                  )}>
                    <div className="absolute top-[-15%] right-[-10%] h-64 w-64 bg-orange-500/10 blur-[100px] rounded-full" />
                    
                    <div className="flex items-center justify-between mb-8">
                      <div>
                        <h2 className="text-3xl font-black text-orange-500 mb-1">{roastData.card_title}</h2>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-widest">Judgment for @{username}</p>
                      </div>
                      <div className="text-right">
                        <div className="text-3xl font-black">{roastData.roast_score}%</div>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-widest">Humiliation</p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <p className="text-center text-sm font-bold text-muted-foreground uppercase tracking-widest">Send the Roast</p>
                    
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <a href={tagTweetUrl} target="_blank" rel="noreferrer"
                        ...
                        <Flame className="h-5 w-5 text-orange-500" /> Tag @{username} on X
                      </a>
                      <a href={challengeTweetUrl} target="_blank" rel="noreferrer"
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
                    ...
                    <p className="text-muted-foreground text-sm mb-4">Think you can handle it yourself?</p>
                    <button onClick={() => { setRoastData(null); setUsername(""); }}
