...
function AdminDashboard() {
  ...
  if (isLoading) return <div className="min-h-screen bg-background p-8 animate-pulse" />;

  // Prepare Chart Data
  const securityTrend = data?.securityEvents.reduce((acc: any[], event) => {
    ...
    <div className="min-h-screen bg-background text-foreground font-sans p-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative z-10">
        ...
        <header className="flex justify-between items-end">
          <div>
            <h1 className="text-6xl font-black tracking-tighter gradient-text">Command Center</h1>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          <div className="xl:col-span-2 glass-morphism rounded-[2rem] border-white/5 overflow-hidden">
            <div className="p-6 border-b border-white/5 flex justify-between items-center">
              ...
              <span className="text-[10px] font-mono text-muted-foreground">LAST 100 EVENTS</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-white/5">
                  <tr>
                    <th className="p-4 text-[10px] font-bold uppercase text-muted-foreground">Event</th>
                    <th className="p-4 text-[10px] font-bold uppercase text-muted-foreground">Source</th>
                    <th className="p-4 text-[10px] font-bold uppercase text-muted-foreground">Time</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {data?.securityEvents.slice(0, 8).map((e, i) => (
                    <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                      <td className="p-4 text-xs font-bold text-blue-400 uppercase">{e.type}</td>
                      <td className="p-4 font-mono text-[11px] opacity-60">{e.ip}</td>
                      <td className="p-4 text-[11px] text-muted-foreground">{new Date(e.timestamp).toLocaleTimeString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="glass-morphism p-8 rounded-[2rem] border-white/5 space-y-6">
            <h3 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Health Signals</h3>
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl">
                ...
                <span className="text-xs font-mono font-bold text-blue-400">10/10</span>
              </div>
              <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl">
                <span className="text-xs font-medium">Redis Cluster</span>
                <span className="text-xs font-mono font-bold text-purple-400">HEALTHY</span>
              </div>
            </div>
          </div>
        </div>
