        ...
        content: [
          {
            ...
            text: JSON.stringify({
              verified: true,
              reason: "Claim matches the diff content",
            }),
          },
        ],
      }),
...
describe("Layer 2: Graph Metrics", () => {
  ...
  it("should handle a star topology (many importers → one core)", () => {
    ...
    const routeNode = metrics.nodeMetrics.find(
      (m) => m.filename === "routes/a.ts",
    );
  ...
  it("should resolve relative paths correctly", () => {
    ...
    expect(resolvePath("src/server/auth.ts", "./db", nodes)).toBe(
      "src/server/db.ts",
    );
...
describe("Layer 3: Impact Scoring", () => {
  it("should weight high-PageRank files higher", async () => {
    ...
    const profile = await computeImpactProfile(
      mockPR as EnrichedPR,
      staticMetrics,
      graphMetrics,
    );
    const dbContrib = profile.perFileContributions.find(
      (c) => c.filename === "core/db.ts",
    );
    const leafContrib = profile.perFileContributions.find(
      (c) => c.filename === "leaf.ts",
    );

    expect(dbContrib?.archScoreContribution).toBeGreaterThan(
      leafContrib?.archScoreContribution || 0,
    );
  });
});
      ...
      diffs: [
        {
          ...
          additions: 5,
          deletions: 0,
          patch: "// TODO: fix this hack later",
        },
      ],
      astDiffs: [
        {
...
describe("Layer 6: Semantic Verification", () => {
  it("should verify citations using the mocked LLM", async () => {
    const mockDraft: Partial<NarrativeDraft> = {
      citations: [
        {
          claim: "Refactored the auth middleware",
          ref: "src/auth.ts",
          sha: "abc123",
  ...
  it("should compute self-consistency as ratio of verified citations", () => {
    ...
    expect(computeSelfConsistency(draft as NarrativeDraft)).toBeCloseTo(
      0.567,
      3,
    );
  });
  ...
  it("should fall back to file presence when LLM and metrics both fail", async () => {
    ...
    (instance.messages.create as any).mockResolvedValueOnce({
      content: [
        {
          type: "text",
          text: JSON.stringify({ verified: false, reason: "No evidence" }),
        },
      ],
    });
