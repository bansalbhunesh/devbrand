...
describe("Engine Robustness & Edge Cases", () => {
  ...
  describe("Layer 2: Graph Logic Edge Cases", () => {
    ...
    it("should handle completely disconnected nodes", () => {
      const graph: DependencyGraph = {
        nodes: [
          { id: "Solo", label: "Solo", type: "source", moduleId: "Solo", isEntryPoint: false, language: "typescript" }
        ],
        edges: [],
        preComputedAt: new Date(),
      };
  ...
  describe("Layer 4: Invisible Work Heuristics", () => {
    it("should not detect refactoring on purely additions", () => {
      const mockPR: Partial<EnrichedPR> = {
        diffs: [{ filename: "new.ts", status: "added", additions: 100, deletions: 0, patch: "many lines" }],
        astDiffs: [{ filename: "new.ts", beforeSymbols: [], afterSymbols: [], addedSymbols: [], removedSymbols: [], changedSymbols: [], addedImports: [], removedImports: [], semanticChange: "none" }]
      };
      const metrics = analyzeStaticMetrics(mockPR as EnrichedPR);
      const report = analyzeInvisibleWork(mockPR as EnrichedPR, metrics);
      expect(report.categories.find(c => c.category === 'refactoring')).toBeUndefined();
    });
    ...
    it("should detect massive cleanups (Net Negative Churn)", () => {
      ...
      const metrics = analyzeStaticMetrics(mockPR as EnrichedPR);
      const report = analyzeInvisibleWork(mockPR as EnrichedPR, metrics);
      expect(report.categories.find(c => c.category === 'tech_debt')).toBeDefined();
    });
  });
});
