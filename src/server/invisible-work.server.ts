...
export function classifyWork(
  prTitle: string,
  _filenameList: string[],
  diffSize: { added: number; deleted: number },
): InvisibleWorkSignal {
  ...
  if (title.includes("fix") || title.includes("bug")) {
    ...
    if (diffSize.deleted > diffSize.added * 2) {
      ...
      invisibleStory =
        "Improved codebase health by aggressively removing redundant logic.";
    }
    ...
    isInvisibleWork = true;
    invisibleStory =
      "Executed a critical platform migration that unblocks future scale.";
