...
export function ollamaRootFromEnv(): string {
  // We keep some process.env for local-only OLLAMA vars to avoid cluttering the production schema
...
export function resolveOpenAiCompatModel(): string | null {
  const m = env.OPENAI_COMPAT_MODEL?.trim() || env.GEMINI_MODEL?.trim();
  if (m && m.length > 0) return m;
  if (env.GEMINI_API_KEY?.trim()) {
    return env.DEFAULT_GEMINI_MODEL?.trim() || "gemini-2.0-flash";
  }
  return null;
}
