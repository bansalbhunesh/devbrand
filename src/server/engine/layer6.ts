...
async function verifySemanticClaim(
  ...
  try {
    ...
    return {
      verified: false,
      reason: "Semantic engine error or timeout.",
      confidence: 0,
    };
  }
}
...
export async function verifyCitations(
  ...
  const verifiedCitations = await Promise.all(
    draft.citations.map(async (citation) => {
      ...
      const fileExists = enrichedPR.diffs.some(
        (d) => d.filename === citation.ref,
      );
      ...
      if (diff && citation.claim) {
        const result = await verifySemanticClaim(
          citation.claim,
          diff.patch || "",
        );
        if (result.verified) {
      ...
      if (!verified && citation.evidenceType === "metric") {
        const metric = staticMetrics.fileMetrics.find(
          (m) => m.filename === citation.ref,
        );
        if (metric) {
          if (
            citation.claim.toLowerCase().includes("complexity") &&
      ...
      if (!verified && fileExists) {
        ...
        verified = true;
        details =
          "File presence verified, but content-level grounding is unverified.";
        confidence = 0.3;
      }
...
export function computeSelfConsistency(draft: NarrativeDraft): number {
  const weightedVerified = draft.citations.reduce(
    (acc, c) => acc + weightCitationConfidence(c),
    0,
  );
  return draft.citations.length > 0
    ? weightedVerified / draft.citations.length
    : 1;
}
...
export async function runLayer6(
  ...
  const consistencyScore = computeSelfConsistency({
    ...draft,
    citations: verifiedCitations,
  });

  const metricVerified = verifiedCitations.filter(
    (c) => c.evidenceType === "metric" && c.verified,
  ).length;
  const structuralVerified = verifiedCitations.filter(
    (c) => c.evidenceType === "structural" && c.verified,
  ).length;

  const varietyBonus = metricVerified > 0 && structuralVerified > 0 ? 0.2 : 0;
  const evidenceDensityScore = Math.min(
    1,
    verifiedCitations.filter((v) => v.verified).length / 5 + varietyBonus,
  );
