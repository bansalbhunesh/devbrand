...
vi.mock("../db", () => ({
  db: { query: { repoGraphs: { findFirst: vi.fn() } }, insert: vi.fn().mockReturnValue({ values: vi.fn().mockReturnValue({ onConflictDoUpdate: vi.fn() }) }) },
}));
vi.mock("../schema", () => ({
...
describe("Layer 2: Path Alias Resolution", () => {
  ...
  it("should resolve alias imports against node list", () => {
    const nodes = ["src/lib/utils.ts", "src/server/db.ts", "src/components/Nav.tsx"];
    expect(resolvePath("src/routes/index.ts", "@/lib/utils", nodes)).toBe("src/lib/utils.ts");
  ...
  it("should handle fuzzy basename matching", () => {
    ...
    expect(resolvePath("src/routes/index.ts", "some-pkg/unique-helper", nodes)).toBe("src/deep/nested/unique-helper.ts");
  });
  ...
  it("should NOT fuzzy match when ambiguous", () => {
    ...
    expect(resolvePath("src/routes/index.ts", "some-pkg/utils", nodes)).toBeNull();
  });
});
