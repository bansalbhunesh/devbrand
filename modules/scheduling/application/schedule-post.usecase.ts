...
import { scheduledPosts, backgroundJobs, outputs } from "@infrastructure/database/schema.server";
import { and, eq } from "drizzle-orm";

const VALID_CHANNELS = new Set(["linkedin", "twitter"]);
const VALID_POST_KINDS = new Set(["linkedinPost1", "linkedinPost2", "linkedinPost3", "twitterThread"]);

export class SchedulePostUseCase {
  async execute(userId: string, data: { outputId: string; channel: string; postKind: string; scheduledFor: string }) {
    if (!VALID_CHANNELS.has(data.channel)) throw new Error("INVALID_CHANNEL");
    if (!VALID_POST_KINDS.has(data.postKind)) throw new Error("INVALID_POST_KIND");
