  ...
  async execute(teamId: string) {
    ...
    const avgImpact =
      recentImpacts.length > 0
        ? recentImpacts.reduce(
            (sum: number, o: any) => sum + (o.impactScore || 0),
            0,
          ) / recentImpacts.length
        : 0;

    const coreInfraCount = recentImpacts.filter(
      (o: any) => o.category === "Architecture" || (o.impactScore || 0) > 80,
    ).length;
    ...
    const membersWithStats = members.map((m: any) => {
      const userImpacts = recentImpacts.filter(
        (o: any) => o.userId === m.userId,
      );
