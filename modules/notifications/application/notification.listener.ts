...
export class NotificationListener {
  ...
  init() {
    ...
    this.eventBus.on("TRANSFORM_COMPLETED", async (event) => {
      ...
      if (user) {
        console.log(`[Notification] Sending 'Transformation Success' to ${user.githubLogin}: Your PR is ready at /t/${slug}`);
    ...
    this.eventBus.on("TRANSFORM_FAILED", async (event) => {
      ...
      if (user) {
        console.log(`[Notification] Sending 'Transformation Failed' to ${user.githubLogin}: Error: ${error}`);
      }
    });
    ...
    this.eventBus.on("POST_SCHEDULED", async (event) => {
      ...
      if (user) {
        console.log(`[Notification] Confirmation sent to ${user.githubLogin}: Post scheduled for ${channel}.`);
      }
    });
  }
}
